...
|---|---|
| 入口画面の表示 | 毎回表示（全アクセス時） |
| アート | ツイート動画と同一の emojicombos 大判 Braille アート（放射状まつ毛の目、100桁×50行）を流用・加工 |
...
ツイート動画で使われているものと同一の大判 Braille アート（emojicombos の eye-ascii-art、
放射状まつ毛の目、100桁×50行）をベースにする。点字文字を 2×4 ドットに分解し、
虹彩円（上下まぶたと融合している上端・下端を除いた、動画でピンクに光る領域）を
楕円判定で切り出して「瞳」レイヤーに、残りを「輪郭」レイヤーにする。
加工は使い捨てスクリプト＋手調整で行い、結果のテキストのみ `eyeArt.ts` にコミットする
（実装計画の作成時に加工済み。完成データは実装計画の Task 1 に埋め込んである）。
