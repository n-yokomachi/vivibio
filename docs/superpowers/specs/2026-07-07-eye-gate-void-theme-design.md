...
| 周囲（グレー） | 動画の1ループ 246フレーム（20fps・12.3秒）を完全再生。波・明滅は動画と同一 |
| 瞳の動き | 動画から抽出した視線状態パターン6種をカーソル方向で切替 + 平行移動 + 穴クリップ。lerp 追従、アイドル時は自動で視線が泳ぐ |
| 瞬き | 再生クロックに同期（ループ内 1.7s / 9.5s、実測エンベロープで潰す）。グレー側の瞼閉じ（再生データ内）と完全同期 |
...
2. **周囲グレー**: ループ 246 フレームを格子サンプリング（カーソル=高輝度無彩色塊と下部メニュー行は除外）。
   マスタセル一覧 + 初期ビット列 + フレームごとの XOR デルタ（base36）にエンコード
...
- **周囲**: rAF ループで再生クロック（20fps・12.3s ループ）に従いデルタを適用して全フレーム再生。
  色はフラット `rgb(76,76,76)`
