...
**Interfaces:**
- Consumes: なし
- Produces: `EYE_OUTLINE: string`（目の輪郭レイヤー、64桁×26行の点字アート）、`EYE_PUPIL: string`（瞳レイヤー、同一グリッド）。全行が U+2800 で64桁に揃えてある。Task 4 の EyeGate が import する
...
// Source: https://emojicombos.com/eye-ascii-art
// Split into a static outline layer and a movable pupil layer at
// braille-dot level. Both layers share the same 64x26 character grid
// (every line padded to 64 chars with U+2800) so they align exactly
// when stacked.
...
Expected: `01c99488b84ebceb91f2876b11acde55`
...
for name, art in zip(['EYE_OUTLINE', 'EYE_PUPIL'], arts):
    lines = art.split('\n')
    assert len(lines) == 26, f'{name}: {len(lines)} lines (expected 26)'
    for i, l in enumerate(lines):
        assert len(l) == 64, f'{name} line {i}: {len(l)} chars (expected 64)'
...
<style>
  ...
  /* 目の開口部。瞳がまぶたからはみ出さないよう楕円で切る。
     中心・半径はアートの虹彩位置（中心 x49.2% y48%）に合わせてある。 */
  .socket {
    position: absolute;
    inset: 0;
    clip-path: ellipse(16% 15% at 49.2% 48%);
  }
  .art-pupil {
    color: var(--void-accent);
  ...
  if (gate && eye && pupil && enterBtn) {
    ...
    if (!reduced) {
      ...
      const tick = (t: number) => {
        ...
        // 3.2% / 2.4% は socket の楕円内に収まるよう調整した可動域。
        let dx = cx * 2 - 1;
        let dy = cy * 2 - 0.9;
        ...
        pupil.style.setProperty('--px', (dx * 3.2).toFixed(2) + '%');
        pupil.style.setProperty('--py', (dy * 2.4).toFixed(2) + '%');
...
調整が必要な場合（勘所）:
- 瞳のはみ出し・可動域: script の `3.2` / `2.4`（%）と style の `.socket` の `ellipse(16% 15% at 49.2% 48%)` を対で調整
